...
func TestCreateAgents(t *testing.T) {
	...
	if agent.entity.Metadata.Name != "10.0.0.1:8301" {
		t.Errorf("Expected Entity name '10.0.0.1:8301' got %s", agent.entity.Metadata.Name)
	} else if agent != leader {
		t.Error("Leader was no correctly set")
	}
}

func TestLeaderReportsItselfWithLocalAddress(t *testing.T) {
	mux, hostname, port, serverClose := testutils.SetupServer()
	defer serverClose()
		...
		fmt.Fprint(w, `[
			...
			},
			{
				...
				"Tags": {
					"build": "1.2.1:39f93f01",
					"dc": "dev",
					"id": "c7f88fba-f8d9-94a9-3627-523398acf7da",
					"port": "8300",
					"raft_vsn": "3",
					"role": "consul",
					"segment": "",
	...
	agent := agents[0]
	if agent.entity.Metadata.Name != "127.0.0.1:8301" {
		t.Errorf("Expected Entity name '127.0.0.1:8301' got %s", agent.entity.Metadata.Name)
	} else if agent != leader {
		t.Error("Leader was no correctly set")
	}

	if leader.ipAddr == "127.0.0.1" {
		t.Error("leader address cannot be localhost as subsequent NewDatacenter call will fail")
	}
}
